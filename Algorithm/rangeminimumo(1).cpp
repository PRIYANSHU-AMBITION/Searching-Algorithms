...
void preprocess(int arr[], int n)
{
    ...
    for (int j=1; (1<<j)<=n; j++)
    {
        ...
        for (int i=0; (i+(1<<j)-1) < n; i++)
        {
            ...
            // arr[lookup[3][3]]
            //cout<<" "<<(i+(1<<j)-1)<<" second"<<endl;
            cout<<i<<" "<<j-1<<" "<<i + (1<<(j-1))<<" "<<j-1<<endl;
            cout<<lookup[i][j-1]<<" "<<lookup[i+(1<<(j-1))][j-1]<<endl;
            if (arr[lookup[i][j-1]] < arr[lookup[i + (1<<(j-1))][j-1]])
                {
                    lookup[i][j] = lookup[i][j-1];
                    cout<<lookup[i][j]<<" "<<lookup[i][j-1]<<endl;
                }
            else
                {
                    lookup[i][j] = lookup[i + (1 << (j-1))][j-1];
                    cout<<lookup[i][j]<<" "<<lookup[i+(1<<(j-1))][j-1]<<endl;
                }
        }
    }
}
